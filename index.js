      ...
      if (editing) {
        ...
        mappingData.TOFU_2.forEach(element => {
          ...
          overlayDiv.addEventListener("click", () => {
            ...
            if (overlayDiv.style.borderColor === "red") {
              // If red, add the id if it's not already present.
              if (!selected.includes(element.id)) {
                selected.push(element.id);
              }
            } else {
              // If blue, remove the id from the array.
              const index = selected.indexOf(element.id);
              if (index > -1) {
